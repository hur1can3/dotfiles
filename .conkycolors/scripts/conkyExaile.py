            ...
            if self.musicData == None:
                ...
                if self.testDBus(bus, 'org.exaile.Exaile'):
                    ...
                    try:
                        ...
                        coverart = ""
                        try:                         
                            imgString = "".join(chr(byte) for byte in iface.GetCoverData())
                            if imgString:
                                img = Image.open(StringIO(imgString))
                                img.save("/tmp/exaile-coverart.jpg", "JPEG")
                                coverart = "/tmp/exaile-coverart.jpg"
                        except:
                            coverart = ""
...
def main():
    ...
    if options.version == True:
        print >> sys.stdout,"conkyExaile v.2.11"
    else:
